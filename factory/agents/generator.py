...
async def run_staff_review(
    ...
    prompt = (
        ...
        "   - Consolidate bloated/duplicate test files (name by feature, "
        "use parametrize)\n"
        "   - Add missing type hints\n"
        "5. **Update context files**:\n"
        "   - Update `ARCHITECTURE.md` if components were added or changed\n"
        "   - Update/create `CONTEXT.md` in every module that was modified\n"
        "   - Update `CHANGELOG.md` with what was done in this issue\n"
        "6. **Do NOT**: rewrite working code for style preferences, "
        "add unnecessary abstractions, or change the architecture\n"
        "7. Run `make test` and `make check` after changes — "
        "everything must still pass\n\n"
        "Keep changes surgical. If the code is already good, say so and "
        "make no changes. Don't change tests unless they have bugs or bloat.\n\n"
        f"---\n\n"
        f"## Original Issue\n\n"
        f"**Title**: {issue_title}\n\n"
        f"**Requirements**:\n{issue_body}\n"
